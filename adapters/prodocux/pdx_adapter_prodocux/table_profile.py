"""``prodocux.table_profile`` — Kernel tabular-intake adapter."""
...
TOOL_ID = "prodocux.table_profile"
MAX_TABLE_BYTES = 8 * 1024 * 1024


class TableProfileExecutor:
    ...
    def execute(
        ...
        return {
            ...
            "artifacts": [
                {
                    "name": "table_profile.json",
                    "uri": f"artifact://{TOOL_ID}/table_profile.json",
    ...
    def run(self, inputs: dict[str, Any], output_dir: Path) -> dict[str, Any]:
        path = Path(str(inputs.get("table_path") or ""))
        if not path.is_file():
            raise ValueError("table_path must identify an existing file")
        if path.suffix.casefold() != ".csv":
            raise ValueError("table_path must end with .csv")
        raw = path.read_bytes()
        if len(raw) > MAX_TABLE_BYTES:
            raise ValueError(f"CSV exceeds {MAX_TABLE_BYTES} bytes")
        response = self.client.profile_table(
            document_b64=base64.b64encode(raw).decode("ascii"),
            document_filename=path.name,
        )
        profile = response.get("profile") or {}
        output_dir.mkdir(parents=True, exist_ok=True)
        report = output_dir / "table_profile.json"
        report.write_text(
        ...
        return {
            ...
            "files": [report],
            "outputs": {
                "table_profile.json": report.as_posix(),
                "profile": profile,
            },
        }


def make_table_profile_executor(base_url: str | None = None) -> TableProfileExecutor:
    return TableProfileExecutor(
        ProDocuXHttpClient(
            base_url
